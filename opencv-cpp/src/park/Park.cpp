...
void Park::get_newcandidate(Mat &src)
{
    ...
    int iLowH = 16, iLowS = 0, iLowV = 0, iHighH = 48, iHighS = 255, iHighV = 255;
    ...
    for (int i = 0; i < _contours.size(); i++)
    {
        ...
        if (rect_area >= 10000 && rect_area < 300000) //排除面积小于1000的连通域
        {
	...
	drawContours(contour, _contours, -1, Scalar(200,200,200), 2);
	if(!_squar_index.empty())
	{
	   for(int i = 0;i<_squar_index.size();i++)
	   drawContours(contour, _contours, _squar_index[i], Scalar(255,0,0), 5);
	}
...
int Park::solve_pnp(vector<Point2f> point_img)
{
    ...
    solvePnP(_Point3D, point_img, cameraMatrix, distCoeffs, rvec, tvec, false, SOLVEPNP_ITERATIVE );
